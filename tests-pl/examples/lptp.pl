...
/*
/* Filename: scryer.pl */
/* Abstract: System predicates for Scryer Prolog
...
:- dynamic(io__lptp_home/1).

assert(Fact) :-
%    open('./tmp/facts.log', append, LogStream),
%    write(LogStream, Fact),
%    write(LogStream, '\n'),
%    close(LogStream),
    catch(
        assertz(Fact),
        E,
        (
            write('An error occurred when asserting: '), write(Fact), nl,
            write(E), nl,
            fail
        )
    ).
...
atomic_length(Atom,N) :-
	(	(atomic(Atom), \+ integer(Atom)) ->
		atom_length(Atom,N)
	;	number_chars(Atom,CharL),
		length(CharL,N)
	).

%%d concat_atom(grL::in,grL::out)

as_atom(In, Out) :-
    (   atomic(In)
    ->  In = Out
    ;   integer(In),
        number_chars(In, InChars),
        atom_chars(Out, InChars)
    ;   In = [_|_],
        atom_chars(Out, In)
   ;    write('failed at ensuring In is atomic'),
        fail    ).

concat_atom([],'').
concat_atom([Atom],AtomOut) :- as_atom(Atom, AtomOut).
concat_atom([A1|[A2|Rest]],AtomOut) :-
    as_atom(A1, A1Atom),
    as_atom(A2, A2Atom),
	atom_concat(A1Atom,A2Atom,A12),
	concat_atom(Rest,RestAtom),
	atom_concat(A12,RestAtom,AtomOut).

%%d io__get_stream(gr::in,gr::in,gr::out)

io__get_stream(File,Mode,Alias) :-
    (   atomic(File)
    ->  atom_chars(File, FileChars)
    ;   File = FileChars ),
    time:current_time(T),
    phrase(format_time("%d_%m_%Y_%H%M%S", T), TimestampChars),
    atom_chars(Timestamp, TimestampChars),
    % prevent permission error
    concat_atom([File, Timestamp], Alias),
	open(FileChars,Mode,_Stream,[alias(Alias)]).
...
read_with_variables(Term,VarL) :-
	    ...
	    error(syntax_error(_E), _),
	    (
%	        ctl__warning([cannot,read,'variables:', E]),
%	        ctl__warning([cannot,read,'term:', Term]),
	        VarL = []
	    )
    ),
%    ctl__message(['Term: ',Term]),
%    ctl__message(['VarL: ',VarL]),
    true.
...
io__exec_file(File) :-
        ...
        E,
        (
            write('An error occurred when running io__exec_file/1:'), nl,
            write(consulting(File)), nl,
            write(error(E)), nl,
            io__close_output
        )
    ).
...
io__loop_file(Stream,Term) :-
	(	at_end_of_stream(Stream) ->
	    Term = :-(Goal),
	    write(':- initialization('), writeq(Goal), write(').'),nl,
	    close(Stream)
...
user:term_expansion((:- draft), (:- initialization(draft))).
user:term_expansion((:- pedantic), (:- initialization(pedantic))).
user:term_expansion((:- plain), (:- initialization(plain))).
user:term_expansion((:- show), (:- initialization(show))).
user:term_expansion((:- initialize), (:- initialization(initialize))).

user:term_expansion((:- assert(Fact)), (:- initialization(assert(Fact)))).
user:term_expansion((:- assert_clauses(Prd,ClsL)), (:- initialization(assert_clauses(Prd,ClsL)))).
user:term_expansion((:- assert_fact(Kind,Ref,Form)), (:- initialization(assert_fact(Kind,Ref,Form)))).
user:term_expansion((:- assert_fun(Term,Form1,Form2)), (:- initialization(assert_fun(Term,Form1,Form2)))).
user:term_expansion((:- assert_pred(Form2,Form3)), (:- initialization(assert_pred(Form2,Form3)))).

user:term_expansion((:- definition_pred(Name,N,X)), (:- initialization(definition_pred(Name,N,X)))).
user:term_expansion((:- definition_fun(Name,N,X,existence by Refex,uniqueness by Refuni)),
    (:- initialization(definition_fun(Name,N,X,existence by Refex,uniqueness by Refuni)))).

user:term_expansion((:- io__open_user(Path)), (:- initialization(io__open_user(Path)))).
user:term_expansion((:- io__close_user), (:- initialization(io__close_user))).

user:term_expansion((:- compile_gr(Path)), (:- initialization(compile_gr(Path)))).

user:term_expansion((:- needs_gr(Path)), (:- initialization(needs_gr(Path)))).
user:term_expansion((:- needs_thm(Path)), (:- initialization(needs_thm(Path)))).
user:term_expansion((:- tex_file(Path)), (:- initialization(tex_file(Path)))).
user:term_expansion((:- thm_file(Path)), (:- initialization(thm_file(Path)))).

user:term_expansion((:- axiom(Ref,EForm)), (:- initialization(axiom(Ref,EForm)))).
user:term_expansion((:- theorem(X,Y,Z)), (:- initialization(theorem(X,Y,Z)))).
user:term_expansion((:- lemma(X,Y,Z)), (:- initialization(lemma(X,Y,Z)))).
user:term_expansion((:- corollary(X,Y,Z)), (:- initialization(corollary(X,Y,Z)))).

user:term_expansion((:- bye), (:- initialization(bye))).
user:term_expansion((:- bye(File)), (:- initialization(bye(File)))).

user:term_expansion((:- set(Alias,Path)), (:- initialization(set(Alias,Path)))).

user:term_expansion((:- set(Flag)), (:- initialization(set(Flag)))).
user:term_expansion((:- unset(Flag)), (:- initialization(unset(Flag)))).

user:term_expansion((:- exec(File)), (:- initialization(exec(File)))).
user:term_expansion((:- check(Path)), (:- initialization(check(Path)))).
...
set_lptp_root_dir :-
    io__lptp_home(LPTP_ROOT_DIR),
    ground(LPTP_ROOT_DIR).
set_lptp_root_dir :-
    (   io__lptp_home(LPTP_ROOT_DIR),
        \+ ground(LPTP_ROOT_DIR)
    ;   \+ io__lptp_home(_) ),
    os:getenv("LPTP_ROOT_DIR", ROOT_DIR_CHARS),
    atom_chars(ROOT_DIR, ROOT_DIR_CHARS),
    assertz(io__lptp_home(ROOT_DIR)).

:- initialization(set_lptp_root_dir).
...
pr__step_err(Step,V,Gamma,Form) :-
	(	pr__step(Step,V,Gamma,Form) -> 
		true
	;	i2e__derivation_step(Step,_X),
	    db__flag(debug),
        %log__info('incorrect derivation step', [X]),
...
%        \+ eq__alpha(@(all,BV1,[&,Form1|Form1L]),
%            @(all,BV2,[&,Form2|Form2L])),
%        write(l(@(all,BV1,[&,Form1|Form1L]))), nl,
%        write(r(@(all,BV2,[&,Form2|Form2L]))), nl
	),
...
ctl__write_tex(Kind,Ref,Form,Deriv) :-
	(	db__is_open(tex), 
		...
		->  true
		;   ctl__warning([cannot,print,fact])    )
	; 	true
	).
...
io__open(Suffix,Path) :-
	...
	;   io__expand(Path,File),
		...
		io__get_stream(File_Suffix,write,StreamAlias),
		assert(db__stream(Suffix,StreamAlias)),
		assert(db__is_open(Suffix)),
		current_output(CurrentStream),
		io__set_output(StreamAlias),
		io__preamble(Suffix,Path),
		io__set_output(CurrentStream)
	).
...
db__alias(lptp,Path) :-
    os:getenv("LPTP_ROOT", ProjectRootDir)
    ->  atom_chars(Path, ProjectRootDir)
    ;   io__lptp_home(Path).
...
io__substitute('/'($(Alias),Path1),Path4/Path3) :-
	db__alias(Alias,Path2),
	io__substitute(Path1,Path3),
	io__expand(Path2,Path4).
...
check(Path) :-
    bye,
    % https://www.scryer.pl/time.html#statistics/2
    % CPU time in ms
    statistics(runtime, [Start|_]),
    once(io__consult(Path,pr)),
    statistics(runtime, [End|_]),
    ElapsedTime is (End - Start),
    write('check_with_scryer_in_ms('),
    format("~0f", [ElapsedTime]),
    write(')'), nl.

def(EForm) :-
    atom_chars(EForm, Chars),
    append(Chars, ".", Chars1),
    read_from_chars(Chars1, Term),
    once(ctl__prt_definition(Term)).
